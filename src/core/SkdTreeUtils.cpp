        ...
        long *ids, int *ranks, double *distances, double r)
{
    ...
    std::vector<std::pair<double,int>> globalDistancesRanks(nPoints, std::pair<double, int>(std::numeric_limits<double>::max(), myRank));
    ...
    for (std::size_t ip = 0; ip < nPoints; ip++){
        ...
        long nDistanceEvaluations = static_cast<const bitpit::SurfaceSkdTree*>(tree)->findPointClosestCell(point, pointMaxDistance, &id, &distance);
    ...
    MPI_Aint displacements[3] = {offsetof(bitpit::SurfaceSkdTree::SkdCellInfo, distance),
            offsetof(bitpit::SurfaceSkdTree::SkdCellInfo, rank),
            offsetof(bitpit::SurfaceSkdTree::SkdCellInfo, id)};
    ...
    std::vector<bitpit::SurfaceSkdTree::SkdCellInfo> dri_data(nPoints);
    for (std::size_t ip = 0; ip < nPoints; ip++){
